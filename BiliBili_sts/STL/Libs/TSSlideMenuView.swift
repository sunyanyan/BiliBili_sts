...
@objc protocol TSSlideMenuViewDelegate {
    @objc optional func selectedIndex(index:Int)
}

class TSSlideMenuView: UIView {
    ...
    // 代理
    weak var delegate:TSSlideMenuViewDelegate?
    // menu的标题数组
...
extension TSSlideMenuView {
    ...
    fileprivate func initSubViews() {
        ...
        if let tempArray = titleArray {
            ...
            for i in 0..<(tempArray.count){
                ...
                let tapGes = UITapGestureRecognizer(tap: {[weak self] in
                    ...
                    let visableRect = CGRect(x: delta*CGFloat(i), y: 0, width: (self?.contentScrollView?.tsW)!, height: (self?.contentScrollView?.tsH)!)
                ...
                // 标题颜色的设置
                if i == 0{
                    label.textColor = UIColor.tsRGBA(red: KhightLightColor.red, green: KhightLightColor.green, blue: KhightLightColor.blue, alpha: 1.0)

                }else{
                    label.textColor = UIColor.tsRGBA(red: KnormalColor.red, green: KnormalColor.green, blue: KnormalColor.blue, alpha: 1.0)
                }
            }
        }
        ...
        // 2.计算布局
        let tempStr = "吊"
        let fitHeight = tempStr.tsHeightWithConstrainedWidth(width: 100, font: UIFont.systemFont(ofSize: kfont))
        ...
        for i in 0..<(titleArray.count){
            ...
            if konlyHorizon { // 水平显示（超出边界的情况滑动来显示）
                let fitwidth = str.tsWidthWithConstrainedHeight(height: fitHeight, font: UIFont.systemFont(ofSize: kfont))
                ...
                if maxX > self.tsW{ // 需要换行的情况
                    ...
                    let newPadding = (self.tsW - addWidth)/(CGFloat(i) - 1)
    ...
    fileprivate func countPadding(height:CGFloat) -> CGFloat{
        ...
        for i in 0..<(titleArray!.count){
            let str = titleArray![i]
            let width = str.tsWidthWithConstrainedHeight(height: height, font: UIFont.systemFont(ofSize: kfont))
            addedWidth += width
            // 如果是需要换行的情况下返回一个 设置的padding值
            if addedWidth > self.tsW {
                return Kpadding
            }
        }
        
        let autoPadding = (self.tsW - addedWidth)/(CGFloat(titleArray!.count) - 1)
        return autoPadding
    }
    ...
    fileprivate func menuScrollViewContentSize(height:CGFloat) -> CGSize {
        ...
        return CGSize(width: addedWidth, height: self.tsH)
    }
...
extension TSSlideMenuView{
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if showingIndex == index {// 往右滑动
            leftShowLabel.textColor = UIColor.tsRGBA(red: KhightLightColor.red - percent * kredDelta, green:  KhightLightColor.green - percent * kgreenDelta, blue:  KhightLightColor.blue - percent * kblueDelta, alpha: 1.0)
            rigtShowLabel.textColor = UIColor.tsRGBA(red: KnormalColor.red + percent * kredDelta, green:  KnormalColor.green + percent * kgreenDelta, blue:  KnormalColor.blue + percent * kblueDelta, alpha: 1.0)
        }else{// 往左滑
            rigtShowLabel.textColor = UIColor.tsRGBA(red: KnormalColor.red + percent * kredDelta, green:  KnormalColor.green + percent * kgreenDelta, blue:  KnormalColor.blue + percent * kblueDelta, alpha: 1.0)
            leftShowLabel.textColor = UIColor.tsRGBA(red: KhightLightColor.red - percent * kredDelta, green:  KhightLightColor.green - percent * kgreenDelta, blue:  KhightLightColor.blue - percent * kblueDelta, alpha: 1.0)
        }
        ...
        if leftRect.origin.y == rightRect.origin.y {// 不用换行显示的情况下
            ...
            if konlyHorizon{
                ...
                if showingIndex == index {// 右滑
                    offsetX =  rigtShowLabel.frame.midX - self.tsW/2
                    if percent == 0 {
                        oldMenuScrollviewContentX = leftShowLabel.frame.midX - self.tsW/2
                    }
                    pointX = (offsetX - oldMenuScrollviewContentX)*percent + oldMenuScrollviewContentX
                }else{// 左滑
                    offsetX = leftShowLabel.frame.midX - self.tsW/2
                    if percent == 0 {
                        oldMenuScrollviewContentX = rigtShowLabel.frame.midX - self.tsW/2
                    }
                ...
                // 超过了最大值
                if pointX > (menuBackScrollView.contentSize.width - menuBackScrollView.tsW){
                    pointX = (menuBackScrollView.contentSize.width - menuBackScrollView.tsW)
                }
                // 赋值
